...
duration_sec = 60 * 10;     % simulation duration in seconds
        ...
        for i = 1:numel(leoSats)
            ...
            for gsIdx = 1:numel(leoGsList)
                ...
                if acc
                    ...
                    fprintf('    LEO-%d to %s (%.6f GHz): RSSI=%.2f dBm, SNR=%.2f dB, Throughput=%.2f\n', ...
                        i, leoGsList{gsIdx}.Name, currentLEOFreqs(i)/1e9, rssi, snr, throughput);
                end
            end
        end
...
for i = 1:geoNum
    ...
    for gsIdx = 1:numel(geoGsList)
        ...
        csvData.(sprintf('GEO%d_%s_Thrpt_bits_s', i, gsName)) = logData.GEO(i).Thrpt(:, gsIdx);
    end
end
...
for i = 1:numel(leoSats)
    ...
    for gsIdx = 1:numel(leoGsList)
        ...
        csvData.(sprintf('LEO%d_%s_Thrpt_bits_s', i, gsName)) = logData.LEO(i).Thrpt(:, gsIdx);
    end
end
