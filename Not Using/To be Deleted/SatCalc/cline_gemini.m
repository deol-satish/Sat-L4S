...
duration_sec = 60 * 20;     % simulation duration in seconds
        ...
        for i = 1:leoNum
            ...
            for gsIdx = 1:numel(leoGsList)
                ...
                if true
                    ...
                    channelBW = 200e3; % Channel bandwidth (Hz)
                    snr_linear = 10^(snr/10); % Convert SNR from dB to linear scale
                    channel_capacity = channelBW * log2(1 + snr_linear); % Channel capacity (bits/sec)
                    throughput = channel_capacity; % Assuming throughput equals channel capacity
                    logData.LEO(i).Throughput(sampleCount, gsIdx) = throughput;

                    % Estimate queue delay (M/M/1 queue)
                    traffic_load_factor = 0.8;
                    arrival_rate = traffic_load_factor * throughput;
                    service_rate = throughput;
                    if service_rate > arrival_rate
                        queue_delay = 1 / (service_rate - arrival_rate);
                    else
                        queue_delay = Inf; % Queue delay is infinite if arrival rate exceeds service rate
                    end
                    logData.LEO(i).QueueDelay(sampleCount, gsIdx) = queue_delay;

                    % Estimate packet loss rate
                    snr_threshold = 5; % dB
                    if snr < snr_threshold
                        packet_loss_rate = 1 - (snr / snr_threshold); % Linear model
                    else
                        packet_loss_rate = 0;
                    end
                    logData.LEO(i).PacketLossRate(sampleCount, gsIdx) = packet_loss_rate;

                    fprintf('    LEO-%d to %s (%.6f GHz): RSSI=%.2f dBm, SNR=%.2f dB, Throughput=%.2f bps, Queue Delay=%.2f s, Packet Loss Rate=%.2f\n', ...
                        i, leoGsList{gsIdx}.Name, currentLEOFreqs(i)/1e9, rssi, snr, throughput, queue_delay, packet_loss_rate);
                end
            end
        end
